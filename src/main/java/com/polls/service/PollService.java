...
public class PollService {
    ...
    public PagedResponse<PollResponse> getPollCreatedBy(String username, UserPrincipal currentUser,
                                                        int page, int size) {
        validatePageNumberAndSize(page,size);
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username,UserPrincipal currentUser,int page , int size) {
        ...
        //map Polls to responses containing vote counts and poll creator details
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
    ...
    public PollResponse castVoteAndGetUpdatedPoll (Long pollId, VoteRequest voteRequest,
        ...
        if (poll.getExpirationDateTime().isBefore(Instant.now())) {
            throw new BadRequestException("Sorry ! this Pos has already espired");
        }
        ...
        try {
            ...
            logger.error("User {} has already voted in poll {}",currentUser,pollId);
            throw new BadRequestException("Sorry You have already cast your vote in this poll");
        }

        //vote saved Return the updated Poll Response nor
    ...
    private Map<Long, Long> getPollUserVoteMap(UserPrincipal currenUser,List<Long> pollIds){
        Map<Long,Long> pollUserVoteMap = null;
        if (currenUser != null) {
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn(currenUser.getId(),pollIds);
